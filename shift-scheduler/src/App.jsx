          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 no-print">
              ...
              <div className={`${theme.navActive} text-white rounded-[2.5rem] p-8 shadow-xl flex flex-col justify-center border border-slate-700`}>
                <span className="text-xs font-black opacity-50 uppercase mb-2">本月基準工時目標</span>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex flex-wrap gap-3 items-center bg-white p-2 rounded-2xl border shadow-inner">
                    <div className="px-4 py-1 flex items-center gap-2 border-r border-slate-100">
                      ...
                      <span className="text-xs font-black text-slate-400 uppercase tracking-tighter">快速筆刷 :</span>
                    </div>
                    {shiftTypes.map(s => (
                      <button 
                        key={s.id} 
                        onClick={() => setQuickAssignMode(quickAssignMode === s.id ? null : s.id)}
                        className={`px-6 py-3 rounded-xl text-sm font-black flex items-center gap-2 transition-all ${quickAssignMode === s.id ? `${theme.colors[s.colorIndex % 5]} text-white shadow-lg scale-110` : 'hover:bg-slate-50 text-slate-500'}`}
                      >
                ...
                <table className="w-full border-separate border-spacing-0 text-left print-table">
                  <thead>
                    <tr className="bg-slate-50/50">
                      <th className="sticky left-0 top-0 z-40 bg-white/95 backdrop-blur-md p-6 text-xs font-black uppercase text-slate-400 border-b border-r min-w-[180px]">成員名冊 (⚡一鍵填滿)</th>
                        ...
                        <td className="sticky left-0 z-30 bg-white/95 backdrop-blur-md p-6 border-r border-b shadow-[5px_0_15px_rgba(0,0,0,0.03)]">
                          <div className="flex flex-col gap-3">
                            ...
                            <button 
                              ...
                              className="no-print flex items-center justify-center gap-2 py-2 px-4 rounded-xl bg-slate-100 text-xs font-black text-slate-500 hover:bg-indigo-50 hover:text-indigo-600 transition-all border border-slate-200"
                            >
                          ...
                          return (
                            <td key={day} className={`p-0 border-b border-r border-slate-50 transition-colors`}>
                              <button 
                                ...
                                className={`w-full h-20 flex items-center justify-center text-4xl transition-all ${s ? theme.colors[s.colorIndex % 5] + " text-white shadow-inner" : "bg-transparent hover:bg-black/[0.02]"}`}
                              >
                                {s ? s.emoji : '·'}
                              </button>
                            </td>
